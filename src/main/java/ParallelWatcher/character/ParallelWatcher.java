package ParallelWatcher.character;

import ParallelWatcher.cards.Defend_Spiritor;
import ParallelWatcher.cards.SoulCall;
import ParallelWatcher.cards.Strike_Spiritor;
import ParallelWatcher.relics.SoulCore;
...
import static ParallelWatcher.ParallelWatcherMod.characterPath;
import static ParallelWatcher.ParallelWatcherMod.makeID;

public class ParallelWatcher extends CustomPlayer {
    //Stats
    ...
    public static final int MAX_ORB_SLOTS = 15;

    //Strings
    private static final String ID = makeID("SPIRITOR"); //This should match whatever you have in the CharacterStrings.json file
    ...
    public static class Enums {
        ...
        @SpireEnum
        public static AbstractPlayer.PlayerClass PARALLEL_WATCHER;
        @SpireEnum(name = "SPIRITOR_COLOR") // These two MUST match. Change it to something unique for your character.
        public static AbstractCard.CardColor CARD_COLOR;
        @SpireEnum(name = "SPIRITOR_COLOR")
        @SuppressWarnings("unused")
        public static CardLibrary.LibraryType LIBRARY_COLOR;
    }

    public ParallelWatcher() {
        super(NAMES[0], Enums.PARALLEL_WATCHER,
    ...
    public ArrayList<String> getStartingDeck() {
        ...
        retVal.add(Strike_Spiritor.ID);
        retVal.add(Strike_Spiritor.ID);
        retVal.add(Strike_Spiritor.ID);
        retVal.add(Strike_Spiritor.ID);
        retVal.add(Defend_Spiritor.ID);
        retVal.add(Defend_Spiritor.ID);
        retVal.add(Defend_Spiritor.ID);
        retVal.add(Defend_Spiritor.ID);
        retVal.add(SoulCall.ID);
    ...
    public void doCharSelectScreenSelectEffect() {
        ...
        CardCrawlGame.sound.playA("GHOST_ORB_IGNITE_1", MathUtils.random(-0.2F, 0.2F));
    ...
    public void increaseMaxOrbSlots(int amount, boolean playSfx) {
        if (this.maxOrbs == MAX_ORB_SLOTS) {
            ...
            if (this.maxOrbs + amount > MAX_ORB_SLOTS) {
                amount = MAX_ORB_SLOTS - maxOrbs;
            }
            maxOrbs += amount;
